...
#define RAS_CODE 3
#define TRQ_PCT 100
#define ACC_LIM_RPM_PER_SEC 200 // gantry 1500
#define VEL_LIM_RPM 200         // gantry 1500
...
public:
  ...
  void PrintStats(bool onInit = false) {
    // cout<<"initial pos: "<<m_node->Motion.PosnMeasured.Value()<<endl;
    ...
    // cout<<"after refresh pos: "<<m_node->Motion.PosnMeasured.Value()<<endl;
  }
  ...
  int32 safeFloatToInt(const FloatType& num) {
    // check if float fits into integer
    if (numeric_limits<int32>::digits < numeric_limits<FloatType>::digits) {
      // check if float is smaller than max int
      if ((num < static_cast<FloatType>(numeric_limits<int32>::max())) &&
          (num > static_cast<FloatType>(numeric_limits<int32>::min()))) {
        return static_cast<int32>(num); // safe to cast
      } else {
        std::cout << "Unsafe conversion of value:" << num << std::endl;
        // NaN is not defined for int return the largest int value
        return numeric_limits<int32>::max();
      }
    } else {
      // It is safe to cast
  ...
  // Set the move distance to the given in meters
  void SetMoveRevs_meters(float meters, int option = 0) {
    /*
     * option 0 default is for x and y motors,
     * option 1 is for z axis
     * option 2 is for the YAW
     */
    // m_move.value =
    // (long)(m_node->Info.PositioningResolution.Value())*numRevs;
    // cout<<"option:
    // "<<option<<endl;
    if (option == 1) { // X-Axis
      m_move.value = safeFloatToInt(
          meters * COUNTS_TO_METERS_Z); // converting to no. of steps
    }
    if (option == 2) { // YAW-Axis
      m_move.value = safeFloatToInt(
          meters * RADIAN_TO_COUNTS_YAW); // converting to no. of steps
    }
    if (option == 0) { // X & Y Axis
      m_move.value = safeFloatToInt(
          meters * COUNTS_TO_METERS_XY); // converting to no. of steps
    }
    // std::cout<<meters<<" meter = "<<m_move.value<<" counts"<<std::endl;
    // printf("[%s] m_move.value=%d\n", m_node->Info.UserID.Value(),
    // m_move.value); m_node->Motion.Adv.HeadDistance = m_move.value/4;
    // m_node->Motion.Adv.TailDistance = m_move.value/4;
  }
