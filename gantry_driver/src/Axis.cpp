...
Axis::~Axis() {
  ...
  m_node->EnableReq(
      false); // false, set this to true
             // if not the motor will loose torqur after finishing each move
             // this might also cause slow response totrajectory execution
             // confirmed - 6/21/2022 enable = true causes execution delays
...
void Axis::Enable() {
  if (m_node != NULL) {
    ...
    attnReq.cpm.Ready = 0;
    attnReq.cpm.WasHomed = 0; // default 1
    ...
    if (m_node->Motion.Homing.HomingValid()) {
                        m_node->Motion.Homing.Initiate();
                                ...
                                attnReg theAttn =
    m_node->Adv.Attn.WaitForAttn(attnReq, 30000); if (!theAttn.cpm.WasHomed) {
                                        mnErr eInfo;
                                        eInfo.ErrorCode = MN_ERR_TIMEOUT;
                                        eInfo.TheAddr = m_node->Info.Ex.Addr();
                                        snprintf(eInfo.ErrorMsg,
    sizeof(eInfo.ErrorMsg), "Error: Timed out waiting for home\n"); throw eInfo;
                                }
                }
    */
  }
...
#define USE_HEAD_TAIL_MOVES 0 // default 1
  ...
  moveDuration = (::int32_t)motion.MovePosnHeadTailDurationMsec(
      m_move.value, theStyle.fld.relative == 0, motion.HeadDistance.Value() > 0,
      motion.TailDistance.Value() > 0);
#else
  m_move.type = MG_MOVE_STYLE_NORM_TRIG_ABS; // MG_MOVE_STYLE_NORM_TRIG_ABS
                                             // MG_MOVE_STYLE_NORM_TRIG
  _mgPosnStyle theStyle(m_move.type);
  moveDuration = (::int32_t)m_node->Motion.Adv.MovePosnDurationMsec(
      m_move.value,
      theStyle.fld.relative == 1); // 0 default
  // moveDuration =
  // (::int32_t)m_node->Motion.Adv.MovePosnDurationMsec(m_move.value,
  //            theStyle.fld.relative == 0);
  // moveDuration = (::int32_t)m_node->Motion.Adv.MoveVelDurationMsec

#endif
  // Timeout for move: expected move duration + some time for synch between the
  // nodes
  m_moveTimeoutMs = moveDuration + 20;
  // printf("  [%2d]: moveCounts=%d moveTimeout=%d\n", m_node->Info.Ex.Addr,
  // m_move.value, m_moveTimeoutMs);
}
//																			   *
//******************************************************************************
...
void Axis::Move() {
  ...
  if (m_moveCount % 2 == 0) {
    ...
    m_node->Motion.Adv.MovePosnHeadTailStart(
        m_move.value, theStyle.fld.relative == 0, theStyle.fld.wait == 1,
        m_node->Motion.Adv.HeadDistance.Value() > 0,
        m_node->Motion.Adv.TailDistance.Value() > 0, theStyle.fld.dwell == 1);
  }
#else
  m_node->Motion.Adv.MovePosnStart(
      m_move.value, theStyle.fld.relative == 1, theStyle.fld.wait == 1,
      theStyle.fld.dwell == 1); // theStyle.fld.relative = 0
#endif
...
void Axis::AxisMain(Supervisor* theSuper) {
  ...
  try {
    ...
    attnMask.cpm.WasHomed = 0; // default 1
  ...
  if (!m_quitting) {
    ...
    // m_node->Motion.AddToPosition(-posn); // Note: this is what caused the
    // encoder reading to reset
    m_node->Motion.PosnMeasured.Refresh();
    // PrintStats(true);
  }
  ...
  while (!m_quitting) {
    ...
    // std::cout<<"Enter to move again...."<<std::endl;
    // cin.get();
  } // while (!m_quitting)
