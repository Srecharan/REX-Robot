...
void Supervisor::SuperMain() {
  ...
  try {
    ...
    // cout<<"m_axes superviser: "<<m_axes.size()<<endl;
    for (Uint16 iAxis = 0; iAxis < m_axes.size(); iAxis++) {
  ...
  while (!m_quitting) {
    try {
      ...
      case SUPER_WAIT_FOR_MOVES_SENT:
        ...
        ResetCondition(m_axisSentMove, m_nodesMoved);
        // std::cout << "m_axis size: " << m_axes.size() << std::endl;

        m_axes.at(0)->MyNode()->Port.Adv.TriggerMovesInGroup(1);
        // double check the '0' for multiple SC-HUB
        // m_axes.at(0)->MyNode()->Port.Adv.TriggerMovesInGroup(2);
        // m_axes.at(0)->MyNode()->Port.Adv.TriggerMovesInGroup(3);
        m_state = SUPER_WAIT_FOR_MOVES_DONE;
        break;
      case SUPER_WAIT_FOR_MOVES_DONE:
        WaitForCondition(m_nodesDone);
        //std::cout << "printing stat..." << std::endl;
        PrintStats();
        // Take a break
        ros::param::set("/move_done", true);
        m_sysMgr.Delay(1000); // 200
        if (m_quitting)
          continue;
        ResetCondition(m_axisDone, m_nodesDone);
        m_state = SUPER_WAIT_FOR_IDLE;
